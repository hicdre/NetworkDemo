...
void IOThread::RegisterIOHandler(HANDLE file, IOHandler* handler)
{
...
ULONG_PTR IOThread::HandlerToKey(IOHandler* handler, bool has_valid_io_context)
{
...
IOThread::IOHandler* IOThread::KeyToHandler(ULONG_PTR key, bool* has_valid_io_context)
{
...
void IOThread::Start()
{
	...
	{
		should_quit_ = false;
		event_thrad_start_ = CreateEvent(NULL, TRUE, FALSE, NULL);
		thread_ = (HANDLE)_beginthreadex(0, 0, IOThreadMain, this, 0, 0);
		::WaitForSingleObject(event_thrad_start_, INFINITE);
		::CloseHandle(event_thrad_start_);
		event_thrad_start_ = NULL;
	}
}
...
void IOThread::Wait(DWORD timeout)
{
...
unsigned IOThread::IOThreadMain(void* params)
{
	reinterpret_cast<IOThread*>(params)->Run();
	return 0;
}
...
bool IOThread::WaitForIOCompletion(DWORD timeout, IOHandler* filter)
{
	IOItem item;
...
bool IOThread::MatchCompletedIOItem(IOHandler* filter, IOItem* item)
{
...
bool IOThread::GetIOItem(DWORD timeout, IOItem* item)
{
...
bool IOThread::ProcessInternalIOItem(const IOItem& item)
{
	if (this == reinterpret_cast<IOThread*>(item.context) &&
		this == reinterpret_cast<IOThread*>(item.handler)) {
